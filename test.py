...
def main():
    ...
    val_transform_det = trans.Compose([trans.Scale(256,256)])  # rescale
    val_data = dates.Dataset(DATA_PATH, DATA_PATH, DATA_PATH + 'test1.txt', 'val', transform=True, transform_med=val_transform_det)
    # num_workers
    val_loader = Data.DataLoader(val_data, batch_size=1, shuffle=False, num_workers=0, pin_memory=True)
    ...
    model = model.cuda()
    # 路径自定
    save_change_map_dir = 'checkpoints\prediction\contrastive_loss\changemaps'
    save_roc_dir = 'checkpoints\prediction\contrastive_loss\roc'
    time_start = time.time()
    # validate会打印f_score，因此返回值没啥用
